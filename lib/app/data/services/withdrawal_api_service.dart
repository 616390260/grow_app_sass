...
import 'package:do_task_project/domain/entities/withdrawal_record.dart';
import 'package:do_task_project/domain/entities/withdrawal_setting.dart';
...
class WithdrawalApiService {
  ...
  Future<Map<String, dynamic>> submitWithdrawal(Map<String, dynamic> params) async {
    try {
      print('请求提现API: $_withdrawalEndpoint, 参数: $params');
  ...
  Future<WithdrawalSetting> getWithdrawalSetting() async {
    try {
      print('请求提现配置API: $_getWithdrawalSettingEndpoint');
      
      final responseData = await _httpService.get<Map<String, dynamic>>(
        _getWithdrawalSettingEndpoint,
      );
      
      print('获取到提现配置响应: $responseData');
