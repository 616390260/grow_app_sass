...
class ErrorHandlerCenter {
  ...
  /// 处理错误码 - 直接抛出异常
  ///
  /// [responseBody] 为完整业务 JSON（如 `{ code, msg, data }`）时，会从中解析
  /// [ApiException.retryAfterSeconds]，供上层做冷却倒计时等交互。
  ...
  ///
  /// 识别以下字段（优先级按数组顺序）：
  /// `retryAfter` / `retrySeconds` / `waitSeconds` / `wait` / `cooldown` /
  /// `coolDown` / `nextRefreshIn` / `nextRequestIn`。
  /// 结果会被夹在 `[1, 3600]` 范围内。
  ...
  /// 从 Dio 响应（HTTP 头 `Retry-After` 或响应 body）解析重试秒数。
