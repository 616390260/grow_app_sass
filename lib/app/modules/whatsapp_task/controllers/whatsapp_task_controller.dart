...
class WhatsappTaskController extends BaseController {
  ...
  /// 教程区展示类型：'video' 或 'image'（同一占位尺寸）
  final mediaKind = 'video'.obs;
  ...
  /// 视频初始化已结束（成功或放弃），用于结束「加载中」占位，避免 Web 上
  /// `initialize()` 永不返回导致页面卡死。
  ...
  /// 启动异步视频加载链（接口 URL → 兜底 MP4），带超时与代际取消，防止悬挂 Future。
